...
== Deliverable ==
La prima identità del sistema (che è all'inizio la ''principale'') viene creata dal modulo nel costruttore di !IdentityManager. Quando il costruttore ha finito, l'utilizzatore del modulo può da subito invocare il metodo ''get_main_id'' di !IdentityManager per ottenere il NodeID relativo a tale identità.
 ...
 * ''Object get_identity_module(NodeID id, string name)''.
 . Recupera un oggetto associato in precedenza ad un nome in una identità. Abortisce il programma se l'identità indicata non è del sistema corrente. Abortisce il programma se a quel nome non era stato in precedenza associato alcun oggetto.
 ...
 * ''Gee.List<IIdmgmtIdentityArc> get_identity_arcs(IIdmgmtArc arc, NodeID id)''.
 . Dato un arco ''arc'' (che quindi identifica anche un sistema diretto vicino ''b'') e una identità ''id'' nel sistema corrente, restituisce i dati di tutti gli ''archi-identità'' formati su ''arc'' che partono dall'identità ''id''.
 ...
 * ''void prepare_add_identity(int migration_id, NodeID old_id)''.
 . Prepara il modulo ad avviare una concertazione coi diretti vicini per la produzione di una nuova identità del sistema basata su una identità precedente.
 . Si vedano sotto i dettagli al relativo [[#add_identity|paragrafo]].
 * ''NodeID add_identity(int migration_id, NodeID old_id)''.
 . Attraverso una concertazione coi diretti vicini, realizza la produzione di una nuova identità del sistema basata su una identità precedente.
 . Si vedano sotto i dettagli al relativo [[#add_identity|paragrafo]].
...
<<Anchor(associazioni)>>
=== Associazioni ===
L'elenco ''dev_list'' dei nomi delle interfacce di rete reali attualmente gestite dal sistema è tenuto in una lista (!ArrayList) di stringhe.

L'elenco ''arc_list'' degli archi attualmente realizzati dal sistema con i suoi vicini è tenuto in una lista (!ArrayList) di oggetti che implementano l'interfaccia IIdmgmtArc.
 ...
 * Per ogni arco-identità ''!IdentityArc w~-,,0,,-~'' su ''arc'' che parte dalla identità ''id~-,,j,,-~'':
  * Chiamiamo ''a'' il sistema corrente. Sia ''arc~-,,0,,-~'' l'arco su cui si appoggia ''w~-,,0,,-~''. Sia ''b'' il sistema collegato all'arco ''arc~-,,0,,-~''.
  * Sia ''b~-,,j,,-~'' l'identità del sistema ''b'' collegata a ''w~-,,0,,-~''.
  * Il manager nel sistema ''a'' crea un duplicato ''w~-,,1,,-~'' dell'arco per assegnarlo ad ''id~-,,i,,-~''. Cioè:
   . ''w~-,,1,,-~ = w~-,,0,,-~.copy();''
   . ''identity_arcs(id~-,,i,,-~-arc~-,,0,,-~).add(w~-,,1,,-~);''
  * Il manager nel sistema ''a'' deve comunicare al manager nel sistema ''b'' attraverso l'arco ''arc~-,,0,,-~'' il ''migration_id'' visto prima e le informazioni dell'istanza di !MigrationDeviceData relativa all'interfaccia ''ir(arc~-,,0,,-~)''. In questa stessa comunicazione il sistema ''b'', se la sua identità interessata ''b~-,,j,,-~'' ha partecipato alla stessa migrazione, risponde con l'identificativo della nuova identità basata su ''b~-,,j,,-~'' e le informazioni della pseudo-interfaccia gestita ora da ''b~-,,j,,-~'' relativa all'interfaccia reale su cui ha ricevuto il messaggio.
   ...
   * Il manager nel sistema ''b'' ora verifica se riconosce il ''migration_id'' e se proprio l'identità ''b~-,,j,,-~'' sta partecipando alla stessa migrazione. Lo fa guardando se esiste una relativa istanza di !MigrationData.
  ...
  * Il manager nel sistema ''a'' ora sa se l'identità ''b~-,,j,,-~'' ha partecipato anch'essa alla migrazione. Se sì, il sistema ''a'' conosce i dati ''peer_old_id_new_mac'' e ''peer_old_id_new_linklocal'' che sono riferiti a ''b~-,,k,,-~'', la quale ora gestisce la vecchia interfaccia del sistema ''b'' che prima era gestita da ''b~-,,j,,-~''.
  * Se ''b~-,,j,,-~'' ha partecipato alla migrazione:
   * Il manager nel sistema ''a'' cambia i dati dell'arco assegnato ad ''id~-,,j,,-~'' relativamente all'interfaccia remota:
    * ''w~-,,0,,-~.peer_mac = peer_old_id_new_mac''.
    * ''w~-,,0,,-~.peer_linklocal = peer_old_id_new_linklocal''.
   * Il manager nel sistema ''a'' cambia i dati dell'arco assegnato ad ''id~-,,i,,-~'' relativamente alla ''identità'' remota:
    * ''w~-,,1,,-~.peer_nodeid'' = Il NodeID di ''b~-,,k,,-~''.
  * Il manager nel sistema ''a'' tramite il ''netns-manager'' aggiunge alle tabelle nel network namespace ''namespaces(id~-,,j,,-~)'' la rotta verso ''w~-,,0,,-~.peer_linklocal'' partendo da ''old_id_new_linklocal'' su ''handled_nics(id~-,,j,,-~)(ir(arc~-,,0,,-~)).dev''.
  * Il manager nel sistema ''b'' a sua volta, ora sa che l'identità ''id~-,,j,,-~'' in ''a'' ha migrato e ha dato luogo a ''id~-,,i,,-~''. Sa anche che la sua identità ''b~-,,j,,-~'' aveva un ''arco-identità'' con ''id~-,,j,,-~'' in ''a'' appoggiato sull'arco con ''a'' che parte da ''if_b''. Ovviamente sa anche se l'identità ''b~-,,j,,-~'' ha partecipato anch'essa alla migrazione formando ''b~-,,k,,-~''.
  * Se ''b~-,,j,,-~'' ha partecipato alla migrazione:
   * Il manager nel sistema ''b'' non ha bisogno di fare nulla in questo momento. Le sue variazioni le apporterà di sua iniziativa poiché anche in esso è avvenuta la migrazione di ''b~-,,j,,-~'' in ''b~-,,k,,-~''.
  * Altrimenti:
   * Il manager nel sistema ''b'', in autonomia come accennato sopra, forma un nuovo ''arco-identità'' ''b~-,,j,,-~''-''id~-,,i,,-~'' e lo segnala al suo utilizzatore.
...
=== Aggiunta o rimozione di una interfaccia di rete reale gestita dal sistema ===
 ...
 * Una identità ''b~-,,0,,-~'' di un vicino, la quale ha un arco-identità che la collega ad una identità ''a~-,,0,,-~'' del sistema corrente, ha migrato ed è stata replicata in ''b~-,,1,,-~'', ma ''a~-,,0,,-~'' non ha partecipato alla stessa migrazione. In questo caso il sistema corrente ne viene notificato. Deve aggiungere alle sue associazioni un nuovo arco-identità che collega ''a~-,,0,,-~-b~-,,1,,-~''; ma deve anche usare i dati (MAC address e indirizzo link-local) del vecchio arco ''a~-,,0,,-~-b~-,,0,,-~'' nel nuovo arco ''a~-,,0,,-~-b~-,,1,,-~''; inoltre deve modificare i dati del vecchio arco ''a~-,,0,,-~-b~-,,0,,-~'' come gli sono stati notificati adesso; infine deve aggiungere un collegamento diretto usando il ''netns-manager'' con i nuovi dati ora assegnati al vecchio arco. In questo caso non si chiama il metodo ''add_identity_arc''.
 * Una identità ''a~-,,0,,-~'' del sistema corrente ha migrato ed è stata replicata in ''a~-,,1,,-~''. In questo caso la sequenza di operazioni da fare è un po' diversa ed è stata dettagliata nella descrizione dei metodi ''prepare_add_identity'' e ''add_identity''. In questo caso non si chiama il metodo ''add_identity_arc''.
